...
const mergeWithFileConfig = async (options) => {
  ...
  options.cwd =
    xoConfigPath && path.dirname(xoConfigPath) !== options.cwd
...
const handleTSConfig = async (options) => {
  ...
  if (tsConfigProjectPath) {
    ...
    const searchResults =
      (await tsConfigExplorer.search(options.filePath)) || {};
  ...
  if (options.tsConfig) {
    ...
    options.tsConfig = await recursiveBuildTsConfig(
      options.tsConfig,
      options.tsConfigPath,
    );
  }
  ...
  if (
    ...
    if (Array.isArray(options.tsConfig.exclude)) {
      const exclude =
        options.tsConfig && Array.isArray(options.tsConfig.exclude)
    ...
    hasMatch =
      !micromatch.contains(options.filePath, exclude) &&
  ...
  if (!hasMatch) {
    ...
    options.tsConfig = options.tsConfigPath
      ...
      : TSCONFIG_DEFAULTS;
    options.tsConfigHash = new MurmurHash3(
      stringify(options.tsConfig),
    ).result();
...
const buildXOConfig = (options) => (config) => {
  ...
  config.baseConfig.settings['import/resolver'] =
    gatherImportResolvers(options);
...
const gatherImportResolvers = (options) => {
  ...
  const resolverSettings =
    options.settings && options.settings['import/resolver'];
  if (resolverSettings) {
...
const getOptionGroups = async (files, options) => {
  const allOptions = await Promise.all(
    arrify(files).map(async (filePath) => parseOptions({...options, filePath})),
  );
...
async function recursiveBuildTsConfig(tsConfig, tsConfigPath) {
  ...
  if (
    ...
    (typeof tsConfig.extends === 'string' &&
      tsConfig.extends.includes('node_modules'))
  ) {
    return tsConfig;
  }
...
const tsConfigResolvePaths = (tsConfig, tsConfigPath) => {
  ...
  if (Array.isArray(tsConfig.files)) {
    tsConfig.files = tsConfig.files.map((filePath) =>
      slash(path.resolve(tsConfigDirectory, filePath)),
    );
  }

  if (Array.isArray(tsConfig.include)) {
    tsConfig.include = tsConfig.include.map((globPath) =>
      slash(path.resolve(tsConfigDirectory, globPath)),
    );
  }

  if (Array.isArray(tsConfig.exclude)) {
    tsConfig.exclude = tsConfig.exclude.map((globPath) =>
      slash(path.resolve(tsConfigDirectory, globPath)),
    );
  }
