...
test('`ignores` option without filename', async (t) => {
  await t.throwsAsync(
    ...
    {
      message:
        /The `ignores` option requires the `filePath` option to be defined./u,
    },
  );
});
...
test('JSX support', async (t) => {
  const {results, rulesMeta} = await xo.lintText(
    'const app = <div className="appClass">Hello, React!</div>;\n',
  );
...
test('prevent use of extended native objects', async (t) => {
  ...
  t.true(
    hasRule(results, 'no-use-extend-native/no-use-extend-native', rulesMeta),
  );
});
...
test('disable style rules when `prettier` option is enabled', async (t) => {
  const {results: withoutPrettier, rulesMeta} = await xo.lintText(
    '(a) => {}\n',
    {filePath: 'test.js'},
  );
...
test('lintText() - overrides support', async (t) => {
  ...
  const {results: barResults} = await xo.lintText(
    await fs.readFile(bar, 'utf8'),
    {filePath: bar, cwd},
  );
  t.is(barResults[0].errorCount, 0);

  const foo = path.join(cwd, 'test/foo.js');
  const {results: fooResults} = await xo.lintText(
  ...
  const {results: indexResults} = await xo.lintText(
    await fs.readFile(bar, 'utf8'),
    {filePath: index, cwd},
  );
...
test('find configurations close to linted file', async (t) => {
  ...
  ({results, rulesMeta} = await xo.lintText(
    `console.log([
  2
]);\n`,
...
test('typescript files: four space fail', async (t) => {
  const fourSpacesCwd = path.resolve(
    'fixtures',
    'typescript',
    'child',
    'sub-child',
...
test('deprecated rules', async (t) => {
  const {usedDeprecatedRules} = await xo.lintText(
    "'use strict'\nconsole.log('unicorn');\n",
  );
