...
const hasRule = (results, filePath, ruleId, rulesMeta) => {
  ...
  const hasRuleInResults = result
    ? result.messages.some((x) => x.ruleId === ruleId)
    : false;
  const hasRuleInResultsMeta = rulesMeta
    ? typeof rulesMeta[ruleId] === 'object'
    : true;
...
test('only accepts allowed extensions', async (t) => {
  ...
  const moreExtensionsResults = await xo.lintFiles(mdGlob, {
    extensions: ['md'],
    cwd,
  });
...
test('do not lint gitignored files', async (t) => {
  ...
  t.is(
    results.some((r) => r.filePath === ignored),
    false,
  );
});
...
test('lint negatively gitignored files', async (t) => {
  ...
  t.is(
    results.some((r) => r.filePath === negative),
    true,
  );
});
...
test.failing('enable rules based on nodeVersion', async (t) => {
  const {results, rulesMeta} = await xo.lintFiles('**/*', {
    cwd: 'fixtures/engines-overrides',
  });
...
test('do not lint eslintignored files', async (t) => {
  ...
  t.is(
    results.some((r) => r.filePath === positive),
    true,
  );
  t.is(
    results.some((r) => r.filePath === negative),
...
test('find configurations close to linted file', async (t) => {
  ...
  t.true(
    hasRule(
      results,
      path.resolve(
        'fixtures/nested-configs/child-override/child-prettier-override/semicolon.js',
      ),
      'prettier/prettier',
      rulesMeta,
    ),
...
test.serial('typescript 2 space option', async (t) => {
  const {errorCount, results} = await xo.lintFiles('two-spaces.tsx', {
    cwd: 'fixtures/typescript',
    space: 2,
  });
...
test.serial('typescript 4 space option', async (t) => {
  const {errorCount} = await xo.lintFiles('child/sub-child/four-spaces.ts', {
    cwd: 'fixtures/typescript',
    space: 4,
  });
  t.is(errorCount, 0);
...
test.serial('typescript no semicolon option', async (t) => {
  const {errorCount} = await xo.lintFiles('child/no-semicolon.ts', {
    cwd: 'fixtures/typescript',
    semicolon: false,
  });
  t.is(errorCount, 0);
