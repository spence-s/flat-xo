...
const {__dirname, require, readJson, readJsonSync} = createEsmUtils(
  import.meta,
);
...
test('buildConfig: defaults', (t) => {
  ...
  t.regex(
    slash(config.cacheLocation),
    /[\\/]\.cache\/xo-linter\/xo-cache.json[\\/]?$/u,
  );
...
test('buildConfig: semicolon', (t) => {
  ...
  t.deepEqual(config.baseConfig.rules['semi-spacing'], [
    'error',
...
test('buildConfig: prettier: true', (t) => {
  const config = manager.buildConfig(
    {prettier: true, extends: ['xo-react']},
    {},
  );
  ...
  t.deepEqual(config.baseConfig.rules['prettier/prettier'], [
    'error',
    {
      useTabs: true,
  ...
  t.is(
    config.baseConfig.extends[config.baseConfig.extends.length - 1],
...
test('buildConfig: prettier: true, typescript file', (t) => {
  ...
  t.regex(
    config.baseConfig.extends[config.baseConfig.extends.length - 2],
    /xo-typescript/,
  );
...
test('buildConfig: nodeVersion: >=6', (t) => {
  ...
  t.deepEqual(config.baseConfig.rules['n/no-unsupported-features/es-syntax'], [
    'error',
    {version: '>=6', ignores: ['modules']},
  ]);
  t.deepEqual(
    config.baseConfig.rules['n/no-unsupported-features/node-builtins'],
    ['error', {version: '>=6'}],
...
test('mergeWithPrettierConfig: determine `tabWidth`, `useTabs`, `semi` from xo config', (t) => {
  ...
  const result = manager.mergeWithPrettierConfig(
    {space: 4, semicolon: false},
    {},
  );
  const expected = {
    bracketSpacing: false,
...
test('mergeWithPrettierConfig: throw error is `semi`/`semicolon` conflicts', (t) => {
  t.throws(() =>
    manager.mergeWithPrettierConfig({semicolon: true}, {semi: false}),
  );
  t.throws(() =>
    manager.mergeWithPrettierConfig({semicolon: false}, {semi: true}),
  );

  t.notThrows(() =>
    manager.mergeWithPrettierConfig({semicolon: true}, {semi: true}),
  );
  t.notThrows(() =>
    manager.mergeWithPrettierConfig({semicolon: false}, {semi: false}),
  );
});

test('mergeWithPrettierConfig: throw error is `space`/`useTabs` conflicts', (t) => {
  t.throws(() =>
    manager.mergeWithPrettierConfig({space: false}, {useTabs: false}),
  );
  t.throws(() =>
    manager.mergeWithPrettierConfig({space: true}, {useTabs: true}),
  );

  t.notThrows(() =>
    manager.mergeWithPrettierConfig({space: 4}, {useTabs: false}),
  );
  t.notThrows(() =>
    manager.mergeWithPrettierConfig({space: true}, {useTabs: false}),
  );
  t.notThrows(() =>
    manager.mergeWithPrettierConfig({space: false}, {useTabs: true}),
  );
});
...
test('mergeWithPrettierConfig: throw error is `space`/`tabWidth` conflicts', (t) => {
  ...
  t.notThrows(() =>
    manager.mergeWithPrettierConfig({space: false}, {tabWidth: 4}),
  );
  t.notThrows(() =>
    manager.mergeWithPrettierConfig({space: true}, {tabWidth: 4}),
  );
});
...
test('buildConfig: rules', (t) => {
  ...
  t.deepEqual(
    config.baseConfig.rules['object-curly-spacing'],
    rules['object-curly-spacing'],
  );
});
...
test('findApplicableOverrides', (t) => {
  ...
  t.deepEqual(result.applicable, [{files: '**/f*.js'}, {files: '**/*oo.js'}]);
});
...
test('mergeWithFileConfig: use child if closest', async (t) => {
  ...
  const eslintConfigId = new MurmurHash3(
...
test('mergeWithFileConfig: read engines from package.json', async (t) => {
  ...
  const expected = {
    nodeVersion: enginesConfig.engines.node,
    extensions: DEFAULT_EXTENSION,
    ignores: DEFAULT_IGNORES,
    cwd,
    eslintConfigId,
...
test("mergeWithFileConfig: XO engine options supersede package.json's", async (t) => {
  const cwd = path.resolve('fixtures', 'engines');
  const {options} = await manager.mergeWithFileConfig({
    cwd,
    nodeVersion: '>=8',
  });
  const eslintConfigId = new MurmurHash3().result();
...
test('mergeWithFileConfig: resolves expected typescript file options', async (t) => {
  ...
  const {options} = await manager.mergeWithFileConfig({cwd, filePath});
  const eslintConfigId = new MurmurHash3(path.resolve(cwd, 'package.json'))
    .hash(tsConfigPath)
    .result();
  const expected = {
    filePath,
...
test('mergeWithFileConfig: resolves expected tsx file options', async (t) => {
  ...
  const tsConfigPath = path.resolve(cwd, 'tsconfig.json');
  const tsConfig = await readJson(tsConfigPath);
  const eslintConfigId = new MurmurHash3(path.join(cwd, 'package.json'))
    .hash(tsConfigPath)
    .result();
  const expected = {
    filePath,
...
test('mergeWithFileConfig: correctly resolves relative tsconfigs excluded file', async (t) => {
  ...
  const excludeTsConfigPath = new RegExp(
    `${slash(cwd)}/node_modules/.cache/xo-linter/tsconfig\\..*\\.json[\\/]?$`,
    'u',
  );
  const {options} = await manager.mergeWithFileConfig({
    cwd,
    filePath: excludedFilePath,
  });
...
test('mergeWithFileConfig: uses generated tsconfig if specified parserOptions.project excludes file', async (t) => {
  ...
  const expectedTsConfigPath = new RegExp(
    `${slash(cwd)}/node_modules/.cache/xo-linter/tsconfig\\..*\\.json[\\/]?$`,
    'u',
  );
  const {options} = await manager.mergeWithFileConfig({cwd, filePath});
...
test('mergeWithFileConfig: auto generated ts config extends found ts config if file is not covered', async (t) => {
  ...
  const expectedConfigPath = new RegExp(
    `${slash(cwd)}/node_modules/.cache/xo-linter/tsconfig\\..*\\.json[\\/]?$`,
    'u',
  );
  const expected = {
...
test('applyOverrides', (t) => {
  t.deepEqual(
    manager.applyOverrides('file.js', {
      overrides: [
        {
          files: 'file.js',
          rules: {'rule-2': 'c'},
