...
test('quiet option', async (t) => {
  ...
  const error = await t.throwsAsync(
    main(['--quiet', '--reporter=json', filepath], {cwd}),
  );
...
test('invalid node-engine option', async (t) => {
  ...
  await fs.promises.writeFile(filepath, 'console.log()\n');
  const error = await t.throwsAsync(
    main(['--node-version', 'v', filepath], {cwd}),
  );
...
test('extension option', async (t) => {
  ...
  const {stdout} = await t.throwsAsync(
    main(['--reporter=json', '--extension=unknown'], {cwd}),
  );
  const reports = JSON.parse(stdout);
...
test('invalid print-config flag with stdin', async (t) => {
  ...
  t.is(
    error.stderr.trim(),
    'The `--print-config` flag is not supported on stdin',
  );
});
...
test('print-config flag without filename', async (t) => {
  const error = await t.throwsAsync(() => main(['--print-config']));
  t.is(
    error.stderr.trim(),
    'The `--print-config` flag must be used with exactly one filename',
  );
